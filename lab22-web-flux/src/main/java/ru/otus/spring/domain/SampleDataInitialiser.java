...
class SampleDataInitialiser implements ApplicationListener<ApplicationReadyEvent> {
    ...
    public void onApplicationEvent(ApplicationReadyEvent event) {
        blogRepository.deleteAll()
                .thenMany(reactor.core.publisher.Flux.just("A", "B", "C", "D").map(name -> new Blog(UUID.randomUUID().toString(),name + "@gmail.com")).flatMap(blogRepository::save))
                .thenMany(blogRepository.findAll())
                .subscribe(blog -> log.info("saving " + blog.toString()));
    }
}
